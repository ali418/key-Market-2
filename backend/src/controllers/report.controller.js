...
exports.inventoryStatusReport = async (req, res, next) => {
  try {
    ...
    inventory.forEach(item => {
      // Check if product exists to avoid null reference error
      if (!item.product) {
        console.warn(`Product not found for inventory item ${item.id}`);
      ...
      inventoryByCategory[categoryName].totalQuantity += item.quantity;
      inventoryByCategory[categoryName].items.push({
        id: item.id,
        productId: item.productId,
        productName: item.product.name,
        quantity: item.quantity,
        minStockLevel: item.minStockLevel || item.get('min_quantity'),
        isLowStock: item.quantity <= (item.minStockLevel || item.get('min_quantity')),
      });
    });
