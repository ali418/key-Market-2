...
exports.getAllProducts = async (req, res, next) => {
  try {
    const { q, search, barcode, is_online } = req.query;
    ...
    if (query) {
      // Search by name/description (English/Arabic) case-insensitive
      where[Op.or] = [
        { name: { [Op.iLike || Op.like]: `%${query}%` } },
        { description: { [Op.iLike || Op.like]: `%${query}%` } },
        { name_ar: { [Op.iLike || Op.like]: `%${query}%` } },
        { description_ar: { [Op.iLike || Op.like]: `%${query}%` } },
      ];
    }
    ...
    if (typeof is_online !== 'undefined') {
      ...
      const normalized = String(is_online).toLowerCase();
      where.is_online = normalized === 'true' || normalized === '1';
    }
...
exports.updateProduct = async (req, res, next) => {
  try {
    ...
    // If barcode is being changed, validate uniqueness
    try {
      if (req.body.barcode && req.body.barcode !== product.barcode) {
        const existing = await Product.findOne({ where: { barcode: req.body.barcode } });
        if (existing && String(existing.id) !== String(product.id)) {
          return res.status(409).json({
            success: false,
            message: 'الباركود موجود بالفعل لمنتج آخر. رجاءً استخدم باركود مختلف.',
          });
        }
      }
    } catch (uniqErr) {
      // continue to update if uniqueness check fails unexpectedly
      console.warn('Barcode uniqueness check failed:', uniqErr.message || uniqErr);
    }
