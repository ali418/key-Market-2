...
const Settings = () => {
  ...
  const [settings, setSettings] = useState({
    store: {
      ...
      currency: storeSettings?.currencyCode || 'AED',
  ...
  const handleSaveSettings = async () => {
    try {
      const payload = {
        ...
        // Attempt to resolve symbol from selected currency in list
        currency_symbol: (currencies.find(c => c.code === settings.store.currency)?.symbol) || storeSettings?.currencySymbol || (settings?.store?.currency ? (currencies.find(c => c.code === settings.store.currency)?.symbol) : '$'),
  ...
  // Sync local state with Redux store when storeSettings changes
  useEffect(() => {
    if (storeSettings) {
      setSettings(prev => ({
        ...prev,
