...
const Reports = () => {
  ...
  const loadReports = useCallback(async () => {
    ...
    try {
      ...
      // Set state with processed data (no demo fallbacks)
      ...
      // No demo data on failure; keep empty to reflect actual state
      setSalesData([]);
      setCategoryData([]);
      setTopProducts([]);
      setInventoryData([]);
  ...
  const handleExport = useCallback(() => {
    ...
    // Add summary data
    doc.setFontSize(12);
    doc.text(`${t('reports:totalSales')}: ${formatCurrency(totalSales)}`, 14, 56);
    doc.text(`${t('reports:totalProfit')}: ${formatCurrency(totalProfit)}`, 14, 64);
          ...
          <Card>
            <CardContent>
              ...
              <Typography variant="h4" component="div" color="warning.main">
                {`${currency.symbol} ${totalInventoryValue.toLocaleString()}`}
              </Typography>
              <Typography variant="body2" color="textSecondary" sx={{ mt: 1 }}>
                {t('reports:totalItems')}: {inventoryData.reduce((sum, item) => sum + (Number(item.currentStock) || 0), 0)}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
