...
const Inventory = () => {
  ...
  const handleViewItem = () => {
    if (selectedItem) {
      console.log('Navigating to product view with ID:', selectedItem.productId);
  ...
  const handleAdjustmentSubmit = async () => {
    ...
    if (quantity > 0 && selectedItem && adjustmentReason) {
      try {
        await apiService.adjustInventory(selectedItem.id, {
          quantity,
          type: adjustmentType,
        ...
        console.error('Failed to adjust inventory via API:', e);
      }
    }
  };
    ...
    <Box sx={{ flexGrow: 1 }}>
      ...
      >
        ...
        <MenuItem onClick={handleRestockOpen}>
          ...
          <ListItemText>{t('inventory:restock')}</ListItemText>
        </MenuItem>
        <MenuItem onClick={handleAdjustStockOpen}>
          <ListItemIcon>
            <TrendingDown fontSize="small" />
          </ListItemIcon>
          <ListItemText>{t('inventory:adjustStock')}</ListItemText>
        </MenuItem>
      </Menu>
      ...
      <StockAdjustment
        ...
        item={selectedItem}
        onSuccess={handleAdjustmentSubmit}
      />
    </Box>
  );
};
