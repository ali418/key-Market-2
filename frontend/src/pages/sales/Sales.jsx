...
const Sales = () => {
  ...
  const normalizeSale = (s) => {
    ...
    return {
      ...
      cashier: s.user?.name || s.cashier || '-',
      itemsCount: s.items?.length || 0,
    };
  };

  const calculateKPIs = (salesData) => {
    const today = new Date();
    const startOfMonth = new Date(today.getFullYear(), today.getMonth(), 1);

    // Only include completed sales
    const completed = (salesData || []).filter(s => (s.status || 'completed') === 'completed');

    const todaySales = completed.filter(sale => isSameDay(sale.date, today));
    const monthSales = completed.filter(sale => isAfter(sale.date, startOfMonth) || isSameDay(sale.date, startOfMonth));
  ...
  const buildTrendData = (salesData) => {
    // Last 30 days only, group by day
    const end = new Date();
    const start = subDays(end, 29);
    const completed = (salesData || []).filter(s => (s.status || 'completed') === 'completed' && s.date >= start && s.date <= end);
    const map = new Map();
    for (let i = 0; i < 30; i++) {
    ...
    completed.forEach(s => {
      ...
      if (map.has(key)) map.set(key, map.get(key) + (s.total || 0));
    });
  ...
  const loadSales = async () => {
    try {
      ...
      const mapped = (Array.isArray(data) ? data : []).map(normalizeSale);
      setSales(mapped);
      calculateKPIs(mapped);
      buildTrendData(mapped);
      applyFilters(mapped);
    } catch (e) {
      console.error(e);
