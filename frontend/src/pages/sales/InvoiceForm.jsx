...
const InvoiceForm = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Show pending status notification when saving
        if (values.status === 'pending') {
          setNotification({ open: true, message: 'تم حفظ الفاتورة بحالة: قيد الانتظار', severity: 'success' });
  ...
  const handleBarcodeSearch = (codeRaw) => {
    const code = (codeRaw || '').trim();
    if (!code) return;

    // Try to find product by barcode, id, sku, serial_number or name
    const lower = code.toLowerCase();
    const product = (products || []).find(p => {
      const barcode = p.barcode?.toString() || '';
      const id = p.id?.toString() || '';
      const sku = p.sku?.toString() || '';
      const serial = p.serial_number?.toString() || '';
      const name = p.name?.toString() || '';
      return (
        barcode === code || id === code || sku === code || serial === code ||
        barcode.toLowerCase() === lower || id.toLowerCase() === lower || sku.toLowerCase() === lower || serial.toLowerCase() === lower || name.toLowerCase().includes(lower)
      );
    });

    if (!product) {
      setNotification({ open: true, message: 'المنتج غير موجود للباركود/الكود المدخل', severity: 'error' });
      setBarcodeInput('');
      return;
    }

    // Add product with quantity 1 (or merge if exists)
    const existingIndex = formik.values.items.findIndex(item => String(item.product.id) === String(product.id));
    if (existingIndex >= 0) {
      const updated = [...formik.values.items];
      updated[existingIndex] = {
        ...updated[existingIndex],
        quantity: updated[existingIndex].quantity + 1,
        total: (updated[existingIndex].quantity + 1) * updated[existingIndex].price,
      };
    ...
    setBarcodeInput('');
    setNotification({ open: true, message: `${product.name} تم إضافته للفاتورة`, severity: 'success' });
  };
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      ...
      >
        <MuiAlert elevation={6} variant="filled" severity={notification.severity} onClose={() => setNotification(prev => ({ ...prev, open: false }))}>
