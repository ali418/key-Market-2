...
const Customers = () => {
  ...
  // تحميل العملاء من API عند التحميل الأول
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const normalized = (Array.isArray(data) ? data : []).map((c) => ({
          ...
          memberSince: c.memberSince || c.createdAt || '',
          lastPurchase: c.lastPurchase || '',
        }));
        
        // Load customer sales data for each customer
        const customersWithSales = await Promise.all(
          normalized.map(async (customer) => {
            try {
              const sales = await apiService.getCustomerSales(customer.id);
              ...
              return {
                ...
                totalOrders: saleCount,
                loyaltyPoints: Math.floor(totalAmount / 10) // 1 point for every $10 spent
              };
            } catch (error) {
              console.error(`Error loading sales for customer ${customer.id}:`, error);
              return customer;
            }
          })
        );
                    ...
                    {filteredCustomers
                      ...
                      .map((customer) => (
                        <TableRow key={customer.id}>
                          ...
                          <TableCell align="right">{`${currency.symbol}${customer.totalPurchases.toFixed(2)}`}</TableCell>
                          ...
                          <Grid container spacing={1}>
                            <Grid item xs={6}>
                              ...
                              <Typography variant="body2" fontWeight="bold">
                                {(currency?.symbol || '$')}{customer.totalPurchases.toFixed(2)}
                              </Typography>
                            </Grid>
                            <Grid item xs={6}>
                              <Typography variant="caption" color="text.secondary">
